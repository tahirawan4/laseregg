...
(function( $ ) {
	...
	var Slider = function(element, options) {
		...
		if (parent.hasClass('slider') === true) {
			...
			this.picker = $('<div class="slider">'+
								'<div class="slider-track">'+
									'<div class="slider-selection"'+options.selectionColor+'></div>'+
									'<div class="slider-handle"></div>'+
									'<div class="slider-handle"></div>'+
								'</div>'+
								'<div class="tooltip"><div class="tooltip-arrow"></div><div class="tooltip-inner"></div></div>'+'<div class="slider-rule"></div>' +
		...
		ruler = this.picker.find('.slider-rule');
		rulerlabel = this.picker.find('.slider-rulelabel');
	 ...
	 */
	function showRule(data,opts){
		var opts = options = typeof opts === 'object' && opts;
		var slider =data.slider;
		
		var aa = options.rule.slice(0).reverse();
		aa = aa.slice(0).reverse();
		
		makeRuler(aa);
		function makeRuler(aa){
			ruler.empty();
			rulerlabel.empty();
			for(var i=0; i<aa.length; i++){
				var distance = i*100/(aa.length-1)+'%';
				var span = $('<span></span>').appendTo(ruler);
				span.css('left', distance);
				
				// show the labels
				if (aa[i] != '|'){
					span = $('<span></span>').appendTo(rulerlabel);
					span.html(aa[i]);
